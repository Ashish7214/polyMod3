...
const BASE_PATH = "./circuits/InnovativeGateCircuit/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/InnovativeGateCircuit.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/InnovativeGateCircuitVerifier.sol:Verifier");
